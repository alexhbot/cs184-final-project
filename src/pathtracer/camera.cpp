...
Ray Camera::generate_ray(double x, double y) const {
  ...
  //
    ...
    Vector3D camera_space = Vector3D(camera_x, camera_y, -1.0);
    Vector3D world_space = c2w * camera_space;
    double mag = sqrt(pow(world_space[0], 2) + pow(world_space[1], 2) + pow(world_space[2], 2));
    world_space[0] = world_space[0] / mag;
    world_space[1] = world_space[1] / mag;
    world_space[2] = world_space[2] / mag;
    
    Ray ray = Ray(pos, world_space);
    ray.min_t = nClip;
    ray.max_t = fClip;
